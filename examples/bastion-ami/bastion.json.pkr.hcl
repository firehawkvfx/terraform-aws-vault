...
source "amazon-ebs" "centos7-ami" {
  ...
  ami_name        = "firehawk-base-centos7-{{isotime | clean_resource_name}}-{{uuid}}"
  instance_type   = "t2.micro"
  region          = "{{user `aws_region`}}"
  source_ami_filter {
    filters = {
...
source "amazon-ebs" "ubuntu16-ami" {
  ...
  ami_name        = "firehawk-base-ubuntu16-{{isotime | clean_resource_name}}-{{uuid}}"
  instance_type   = "t2.micro"
  region          = "{{user `aws_region`}}"
  source_ami_filter {
    filters = {
...
source "amazon-ebs" "ubuntu18-ami" {
  ...
  ami_name        = "firehawk-base-ubuntu18-{{isotime | clean_resource_name}}-{{uuid}}"
  instance_type   = "t2.micro"
  region          = "{{user `aws_region`}}"
  source_ami_filter {
    filters = {
...
build {
  ...
  provisioner "shell" {
    inline         = ["if [[ '${var.install_auth_signing_script}' == 'true' ]]; then", "sudo mkdir -p /opt/vault/scripts/", "sudo mv /tmp/sign-request.py /opt/vault/scripts/", "else", "sudo rm /tmp/sign-request.py", "fi", "sudo mkdir -p /opt/vault/tls/", "sudo mv /tmp/ca.crt.pem /opt/vault/tls/", "sudo chmod -R 600 /opt/vault/tls", "sudo chmod 700 /opt/vault/tls", "sudo /tmp/terraform-aws-vault/modules/update-certificate-store/update-certificate-store --cert-file-path /opt/vault/tls/ca.crt.pem"]
  ...
  provisioner "shell" {
    inline         = ["export DEBIAN_FRONTEND='noninteractive'", "sudo apt-get install dialog apt-utils -y", "sudo apt-get -y update", "sudo apt-get install -y git", "sudo apt-get -y install python3", "sudo apt-get -y install python-apt", "sudo apt-get install -y python3-pip", "sudo pip3 install --user boto3", "ls -dltriah /home/ubuntu/.cache/pip/*/", "ls -dltriah /home/ubuntu/.cache/*/", "ls -dltriah /home/ubuntu/*/", "ls -dltriah /home/*/", "ls -dltriah /*/"]
    inline_shebang = "/bin/bash -e"
    only           = ["ubuntu16-ami", "ubuntu18-ami"]
  }
  provisioner "shell" {
    inline = ["sudo yum update -y", "sudo yum install -y git", "sudo yum install -y python python3 python3-pip", "sudo pip3 install --user boto3"]
