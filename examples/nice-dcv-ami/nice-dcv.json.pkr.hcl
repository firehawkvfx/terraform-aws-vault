...
EOFO
  ...
  provisioner "shell" {
    inline = [
      ...
      "cd /tmp; set -o pipefail; sudo /bin/bash ${var.nvidia_driver} -x -s -j 1 || /var/log/nvidia-installer.log; ls -ltriah", #extract
      "folder=$(echo ${var.nvidia_driver} | cut -d '.' -f-3); ls -ltriah $folder; sudo $folder/nvidia-installer --dkms -s --install-libglvnd", # log at /var/log/nvidia-installer.log
      "echo $folder",
